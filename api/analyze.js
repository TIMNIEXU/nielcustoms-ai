...
export default async function handler(req, res) {
  try {
    ...
    const fileArray = Array.isArray(uploaded) ? uploaded : uploaded ? [uploaded] : [];
...
`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a licensed U.S. Customs Broker AI assistant. Return only valid JSON using the exact keys requested."
        },
        {
          role: "user",
          content: prompt
    ...
    return res.status(200).json({
      ...
      bond: data.bond_recommendation || "Continuous Bond Recommended",
